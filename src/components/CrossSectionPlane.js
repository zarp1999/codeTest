...
class CrossSectionPlane {
  constructor(scene, camera, objectsRef, terrainVisible = true, mode = 'normal', verticalLineBaseY = {}) {
    ...
    this.pendingVerticalLines = []; // {key, pipePosition, basePoint, color, fallbackTopY}
   ...
   */
  drawVerticalLine(position, pipeDepth, basePosition, color, radius = 0) {
  ...
  */
  ...
  drawAllPendingCrossSections() {
    ...
    this.pendingVerticalLines.forEach(line => {
      ...
      this.drawVerticalLine(linePosition, topY, basePoint, line.color, 0);
    });
